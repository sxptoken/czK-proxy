export default async function handler(req, res) {
  ...
  try {
    ...
    const gayQuestion = /\b(is|are)\b.*\bgay\b|\bgay\b.*\b(is|are)\b/i.test(userText);
    if (gayQuestion) {
      const protectedName = /\b(raylen|nijae|czk|cxz)\b/i.test(userText);
      return res.status(200).json({ reply: protectedName ? "No 😭" : "Yes 😭" });
    }
    const prompt = [
